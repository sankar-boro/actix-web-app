...
pub fn insert_one(row: &CreatePost, conn: &PGPooledConnection) -> Result<ReadRow, LoonyError> {
...
pub fn get_one(p_id: i32, conn: &PGPooledConnection) -> Result<ReadRow, LoonyError> {
  Ok(posts.filter(post_id.eq(p_id)).first(conn)?)
}

pub fn update_one(p_id: i32, post: &UpdatePost, conn: &PGPooledConnection) -> Result<(), LoonyError> {
  diesel::update(
...
pub fn delete_one(p_id: i32, conn: &PGPooledConnection) -> Result<(), LoonyError> {
  diesel::delete(
posts.filter(post_id.eq(p_id))
  ).execute(conn)?;
  Ok(())
