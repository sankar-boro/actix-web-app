...
pub async fn delete_many(
    conn: web::Data<App>, 
    payload: web::Json<DeleteMany>
) -> Result<HttpResponse, actix_web::Error> {
    let conn = conn.conn_result()?;
    let p: Vec<String> = serde_json::from_str(&payload.data).unwrap();
    let mut _query = format!("DELETE FROM sankar.book WHERE bookId={} AND uniqueId IN (", &payload.bookId);
    for (_i, _d) in p.iter().enumerate() {
        let t =  Uuid::parse_str(_d).unwrap();
        if _i == 0 {
            _query.push_str(&format!("{}", t));
        } else {
            _query.push_str(&format!(", {}", t));
        }
    }
